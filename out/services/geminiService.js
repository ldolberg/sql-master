...
export const autoTagSnippet = async (code, dialect) => {
    try {
...
export const semanticSearch = async (query, snippets) => {
    try {
...
export const checkSqlSafety = async (code, dialect) => {
    try {
        const response = await ai.models.generateContent({
            model: "gemini-3-flash-preview",
...
export const generateDbtModel = async (name, sql, dialect) => {
    try {
        const response = await ai.models.generateContent({
            model: "gemini-3-flash-preview",
...
export const lintAndFormatSql = async (sql, dialect) => {
    try {
        const response = await ai.models.generateContent({
            model: "gemini-3-flash-preview",
...
let activeChat = null;
export const initializeChat = (dialect) => {
...
export const sendChatMessage = async (message, currentSqlContext) => {
    if (!activeChat)
        initializeChat('PostgreSQL');
