...
export async function runTestCase(test) {
    const start = now();
    try {
        switch (test.id) {
            case 't1': {
                const res = await executeSql('SELECT * FROM users', 'PostgreSQL');
                if (res.status !== 'success' || !res.message.includes('PostgreSQL'))
                    throw new Error('Dialect mismatch in result');
                break;
            }
            case 't2': {
                const res = await executeSql('SELECT * FROM dataset.table', 'BigQuery');
                if (!res.rows.some(r => r.dataset_id))
                    throw new Error('BigQuery specific rows not returned');
                break;
            }
            case 't4': {
                const res = await checkSqlSafety('UPDATE users SET name = "bob"', 'MySQL');
                if (res.isSafe)
                    throw new Error('Dangerous update was marked as safe');
                break;
            }
            case 't5': {
                const res = await lintAndFormatSql('select * from users', 'PostgreSQL');
                if (!res.formattedCode.includes('SELECT'))
                    throw new Error('Keywords were not uppercased');
                break;
            }
            case 't6': {
                const res = await generateDbtModel('TestModel', 'SELECT 1', 'Snowflake');
                if (!res.modelSql.includes('{{') || !res.schemaYaml.includes('version'))
                    throw new Error('Invalid dbt structure');
                break;
            }
            case 't7': {
                const res = await autoTagSnippet('SELECT amount FROM transactions', 'PostgreSQL');
                if (!res.category || res.category === 'Uncategorized')
                    throw new Error('AI failed to categorize financial SQL');
                break;
            }
            case 't8': {
